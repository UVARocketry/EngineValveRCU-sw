...
# 2 "<built-in>" 2
# 1 "libpicutil/uart_debug.c" 2
# 1 "libpicutil/uart_debug.h" 1
# 11 "libpicutil/uart_debug.h"
...
# 3 "libpicutil/uart_debug.c" 2




uint8_t tx_ring_buf[(uint8_t)64];
volatile uint8_t head = 0;
volatile uint8_t tail = 0;

void uart_init() {
    U1CON0bits.TXEN = 1;
# 21 "libpicutil/uart_debug.c"
    U1BRGH = 0;
    U1BRGL = 32;
...
void __attribute__((picinterrupt(("irq(28)")))) U1TX_int(void) {
    if (tail == head) {
        PIE3bits.U1TXIE = 0;
    } else {
        U1TXB = tx_ring_buf[tail];
        tail++;
        if (tail >= (uint8_t)64) {
            tail = 0;
        }
